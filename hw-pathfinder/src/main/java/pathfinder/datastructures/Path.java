...
/**
 * This represents an immutable path between two cartesian coordinate elements(with any data type), particularly
...
public class Path<E> implements Iterable<Path<E>.Segment> {
    ...
    // AF(this) =
    //      first element E in the path => start
    //      each "step" along the path between Es => elements of list path, where
    //          path.get(0) is the first step from the start element to an intermediate element, and
    //          path.get(path.size() - 1) is the final step from an intermediate element to the end
    //      total cost along the path => cost
    //      the destination element in this path, opposite the start element => getEnd()
    ...
    /**
     * The element at the beginning of this path.
     */
    private E start;

    /**
     * The ordered sequence of segments representing a path between elements.
     */
    private List<Segment> path;

    /**
     * Creates a new, empty path containing a start element. Essentially this represents a path
     * from the start element to itself with a total cost of "0".
     *
     * @param start The starting element of the path.
     */
    public Path(E start) {
        this.start = start;
        this.cost = 0;
    ...
    /**
     * Appends a new single segment to the end of this path, originating at the current last element
     ...
     * segmentCost}.
     *
     * @param newEnd      The element being added at the end of the segment being appended to this path
    ...
    /**
     * @return The element at the beginning of this path.
     */
    public E getStart() {
        return start;
    }
    ...
    /**
     * @return The element at the end of this path, which may be the start element if this path
     * contains no segments (i.e. this path is from the start element to itself).
     */
    public E getEnd() {
        if(path.size() == 0) {
            return start;
     ...
     * element and ending at the end element. In the case that this path represents a path between
     * the start element and itself, this iterator contains no elements. This iterator does not
     ...
     * both paths are empty, they are only equal if their starting element is equal.
     *
    ...
    /**
     * Segment represents a single segment as part of a longer, more complex path between elements.
    ...
    public class Segment {
         ...
         *
         * @param start The starting element of this segment.
         * @param end   The ending element of this segment.
        ...
        private Segment(E start, E end, double cost) {
            if(start == null || end == null) {
                throw new NullPointerException("Segments cannot have null elements.");
            }
        ...
        /**
         * @return The beginning element of this segment.
         */
        public E getStart() {
        ...
        /**
         * @return The ending element of this segment.
         */
        public E getEnd() {
            return this.end;
        }
